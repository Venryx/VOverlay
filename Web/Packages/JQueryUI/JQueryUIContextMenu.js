...
// ignore clicks, if they only open a sub-menu
		...
		{
			// custom; if descendent has context-menu handler as well, quit this handler func
			var deepestContextMenuElement = $(event.target).plusParents().first(function() { return $(this).data("moogleContextmenu"); })[0];
			if (deepestContextMenuElement && deepestContextMenuElement != this.element[0])
			{
				this._closeMenu();
				return;
			}
...
// Note: we don't want to unbind the 'contextmenu' event
			...
			self.currentTarget = null; // custom added
			if (this.$menu) // custom added
			{
				this.$menu
